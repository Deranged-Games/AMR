...
function OnStartOrLoad()
  ...
  FX_PlayEffectOnBackground("m02_explosion_spray", {0, 0, 1}, 10)
end
...
function Enable_ResearchOptions()
	...
	Player_UnrestrictResearchOption(0, "MultiroleFighterMAXSPEEDUpgrade1")
	...
	Player_UnrestrictBuildOption(g_playerID, "Hgn_defender")
	Player_UnrestrictBuildOption(g_playerID, "Hgn_strikefighter")
	Player_UnrestrictBuildOption(g_playerID, "Hgn_beamfighter")
	Player_UnrestrictBuildOption(g_playerID, "Hgn_lightcarrier")
end
...
function Rule_spawnEscorts()
  NumPlayerMultiroles = Player_GetNumberOfSquadronsOfTypeAwakeOrSleeping(0, "Hgn_multirolefighter")
  NumPlayerInterceptors = Player_GetNumberOfSquadronsOfTypeAwakeOrSleeping(0, "Hgn_Interceptor") + NumPlayerMultiroles
  if (NumPlayerInterceptors < 5) then
  ...
  elseif (NumPlayerInterceptors < 7) then
    SobGroup_Spawn("EscortInterceptors1", "EscortSpawnSphere")
    SobGroup_SwitchOwner("EscortInterceptors1", 0)
...
function Rule_reinforcementWatch()
  if (Player_FillProximitySobGroup("tempSobGroup", 0, "EnemyCarrier", 5000) == 1) then
    ...
    if (SobGroup_Count("BombersNearEnemyCarrier") >= 1) then
  ...
  elseif (convoys_saved == 2) or (convoys_saved == 1 and convoys_destroyed == 1) or (convoys_destroyed == 2) or (convoys_docked == 1) then
    if (ReinforcementEventHasPlayed == 0) then
	Rule_Add("Rule_lockFighterFacilityHealth")
  ...
  elseif (SobGroup_GetHardPointHealth("EnemyCarrier", "Production 1") <= 0.9) or (SobGroup_HealthPercentage("EnemyCarrier") < 0.8) then
    if (ReinforcementEventHasPlayed == 0) then
	Rule_Add("Rule_lockFighterFacilityHealth")
...
function Rule_launchReinforcements()
  if (EventPlaying == 0) then
    ...
    Rule_AddInterval("Rule_DefendersLaunch", 1)
    Rule_AddInterval("Rule_StaggerLaunch1", 4)
    initialReinforcementsLaunched = 1
    Rule_AddInterval("Rule_AttackRemainingConvoys", 4)
...
function Rule_combatSetup()
	...
	SobGroup_DockSobGroupInstant("AttackersBombers", "EnemyCarrier")
	SobGroup_DockSobGroupInstant("AttackersInterceptors", "EnemyCarrier")
	SobGroup_DockSobGroupInstant("Carrier_Defenders", "EnemyCarrier")
	SobGroup_Move(1, "EnemyCarrier", "CarrierMovePoint")
	Rule_Remove("Rule_combatSetup")
end

function Rule_AttackersLaunch1()
	SobGroup_Launch("AttackersInterceptors", "EnemyCarrier")
	Rule_AddInterval("Rule_AttackersLaunch2", 6)
	Rule_Remove("Rule_AttackersLaunch1")
end

function Rule_AttackersLaunch2()
	SobGroup_Launch("AttackersBombers", "EnemyCarrier")
	Rule_AddInterval("Rule_AttackersCombatSetup", 6)
	Rule_Remove("Rule_AttackersLaunch2")
end

function Rule_AttackersCombatSetup()
	SobGroup_Attack(1, "AttackersBombers", "PlayerMothership")
	SobGroup_SetTactics("AttackersBombers", PassiveTactics)
	SobGroup_GuardSobGroup("AttackersInterceptors", "AttackersBombers")
	Rule_Remove("Rule_AttackersCombatSetup")
end

function Rule_DefendersLaunch()
	SobGroup_Launch("Carrier_Defenders", "EnemyCarrier")
	Rule_AddInterval("Rule_AttackersCombatSetup", 6)
	Rule_Remove("Rule_DefendersLaunch")
end

function Rule_DefendersCombatSetup()
	SobGroup_GuardSobGroup("Carrier_Defenders", "EnemyCarrier")
	Rule_Remove("Rule_DefendersCombatSetup")
end
...
function Rule_startAttackOnConvoy1()
	...
	Rule_AddInterval("Rule_AttackersLaunch1", 30)
