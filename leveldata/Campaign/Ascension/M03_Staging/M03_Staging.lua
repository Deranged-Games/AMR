...
function Rule_AI_Control_Off()
	...
	SobGroup_EnterHyperSpaceOffMap("V_AntiCarrierGroup")
...
function Rule_IntelEvent_ShipyardSafe_Complete()
  if (Event_IsDone("IntelEvent_ShipyardSafe") == 1) then
	...
	Player_UnrestrictBuildOption(0, "Hgn_HeavyFighter")
	Player_UnrestrictBuildOption(0, "Hgn_HeavyBomber")
...
function Rule_GiveUpgrades()
	...
	Player_UnrestrictBuildOption(0, "Hgn_GunFrigate")
...
function Rule_IntelEvent_VaygrWave01_Complete()
   if (Event_IsDone("IntelEvent_VaygrWave01") == 1) or (Event_IsDone("IntelEvent_VaygrWave01_NonFRO") == 1) then
	Objective_SetState(obj_01d_Probes_id, OS_Complete)
	Sound_SetMuteActor("")
	g_FROAttackers = 1
...
function Rule_JumpStartShipyardEvents()
   ...
   elseif (g_FROAttackers == 1 and g_ShipyardRepaired == 1 and g_ProbesDead == 0) then
	Rule_AddInterval("Rule_Re_Start_Shidyardevents", 90)
...
function Rule_VaygrShipyard_Attackers_Detected()
  if (SobGroup_PlayerIsInSensorRange("V_Inf_01", 0) == 1) or (SobGroup_PlayerIsInSensorRange("V_Inf_02", 0) == 1) or (SobGroup_PlayerIsInSensorRange("V_Inf_01", 2) == 1) or (SobGroup_PlayerIsInSensorRange("V_Inf_02", 2) == 1) then
    Sound_MusicPlayType("data:sound/music/battle/battle_04", MUS_Battle)
    print("_________MUSIC BATTLE__________")
...
function Rule_InfCarrier_Grps_Arrive()
	...
	SobGroup_ExitHyperSpace("V_AntiCarrierGroup", "vol_V_ACGrp_Exit")
	...
	FOW_RevealGroup("V_AntiCarrierGroup", 1)
...
function Rule_LauchVaygrFrom_Inf_Carrier_02()
	...
	SobGroup_AttackPlayer("V_AntiCarrierGroup", 0)
