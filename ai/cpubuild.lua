...
function DetermineAntiChassisDemand(enemyIndex)
	local kFighterCounterScale = 1.7 --was 2.0 	Altered because AI was putting a whole lot of emphasis on Escort Destroyers and Assault Carriers.
	local kCorvetteCounterScale = 1.4 --was 1.5
	...
	--print("Enemy Chassis Values: Fi: "..(FiValue*kFighterCounterScale).." Co: "..(CoValue*kCorvetteCounterScale).." Fr: "..(FrValue*kFrigateCounterScale))
	--print("Choosen Anti Edits: Fi: "..AFiDemand.." Co: "..ACoDemand.." Fr: "..AFrDemand)
end

function DetermineChassisDemand(enemyIndex)
	local kFighterCounterScale = 1.0
	local kCorvetteCounterScale = 1.3 --Added in to help equalize like in the above function.
	local kFrigateCounterScale = 1.0
	...
	local AFiValue = PlayersMilitary_AntiFighter(enemyIndex, player_max)*kFighterCounterScale
	local ACoValue = PlayersMilitary_AntiCorvette(enemyIndex, player_max)*kCorvetteCounterScale
	local AFrValue = PlayersMilitary_AntiFrigate(enemyIndex, player_max)*kFrigateCounterScale
	...
	--print("Enemy Anti Values: Fi: "..(AFiValue*kFighterCounterScale).." Co: "..(ACoValue*kCorvetteCounterScale).." Fr: "..(AFrValue*kFrigateCounterScale))
	--print("Choosen Chassis Edits: Fi: "..FiDemand.." Co: "..CoDemand.." Fr: "..FrDemand)
end
...
function DetermineSpecialDemand()
	...
	if ((GetNumCollecting() > 9 or numRUs > 1500) and s_militaryPop > 8 and UnderAttackThreat() < -75) then
		ShipDemandAddByClass( eCruiser, 0.5 )
		ShipDemandAddByClass( eBattleCruiser, 1 )
		...
		if (s_race == Race_Vaygr) then
			if (IsResearchDone( BATTLESHIPCHASSIS ) == 1) then
				ShipDemandAdd( VGR_ALKHALID, 2 )
			end
		end
	end
