...
function CpuBuildSS_SpecialSubSystemDemand()
	...
	if(s_selfTotalValue > 160 and s_militaryStrength > -5 and UnderAttackThreat() < -70 and (timeSinceLastSubSysDemand > sg_specialSubSysDelayTime or GetRU() > 3000)) then
		...
		if(productionDemand <= 1.0) then
			local specialSubsystems = {HYPERSPACE,FIRECONTROLTOWER,HYPERSPACEINHIBITOR,SIDETURRET,VENTRALTURRET,DEFENSEFIELDSHIELDMODULE,BUILDPDW1,BUILDPDW2,DEFENSESUITE}
			if (g_LOD < 2) then
				specialSubsystems = {HYPERSPACE,CLOAKGENERATOR,FIRECONTROLTOWER,HYPERSPACEINHIBITOR,ADVANCEDARRAY,CLOAKSENSOR,SIDETURRET,VENTRALTURRET,DEFENSEFIELDSHIELDMODULE,BUILDPDW1,BUILDPDW2,DEFENSESUITE}
			end
			local minNumber = 10
			local maxNumber = 0
...
function CpuBuildSS_ProcessEachBuildShip()
	...
	for i=0, (bcount-1) do
		...
		if (sg_buildBCHyperspace == 1 and BuildShipType( buildShipId ) == kBattleCruiser) then
			aitrace("BC:SS")
			if (BuildShipHasSubSystem( buildShipId, HYPERSPACE ) == 0 and UnderAttackThreat() < -5 and BuildShipCanBuild( buildShipId, HYPERSPACE) == 1) then
...
function DoSubSystemDemand_Hiigaran()
	...
	if numRUs > 500 then
		local numBCs = NumSquadrons(HGN_BATTLECRUISER)
		if numBCs >= 1 then
			SubSystemDemandSet(PULSARARRAY, numBCs )
			SubSystemDemandSet(BCTUBES, numBCs )
		end
		local numCruisers = NumSquadrons(HGN_CRUISER)
		local numWarMachines = NumSquadrons(HGN_CARRIER) + numBCs + numCruisers
		if numWarMachines >= 1 then
			SubSystemDemandSet(SIDETURRET, numWarMachines)
			SubSystemDemandSet(VENTRALTURRET, numWarMachines)
		end
	end
...
function DoSubSystemDemand_Vaygr()
	...
	if numRUs > 1000 then
		local numOpsCarriers = NumSquadrons(VGR_CARRIER_OPS)
		if numOpsCarriers >= 1 then
			SubSystemDemandSet(BUILDPDW2, numOpsCarriers )
		end
		local numCarriers = NumSquadrons(VGR_CARRIER)+numOpsCarriers
		if numCarriers >= 1 then
			SubSystemDemandSet(DEFENSESUITE, numCarriers )
			SubSystemDemandSet(BUILDPDW1, numCarriers )
		end
		local numWarMachines = NumSquadrons(VGR_BATTLECRUISER)+numOpsCarriers
		if numWarMachines >= 1 then
			SubSystemDemandSet(SIDETURRET, numWarMachines )
			SubSystemDemandSet(VENTRALTURRET, numWarMachines )
		end
